...
const createBook= async function (req, res) {
    let data= req.body
    let savedBooksData= await BookModel.create(data)
    res.send({msg: savedBooksData})
}

const bookList= async function (req, res) {
    let namechanged= await BookModel.find().select({bookName:1 , authorName:1, _id:0})
    res.send({msg: namechanged})
}


const getBooksinYear= async function (req, res) {
    let pubYear = req.body.year
    let selectedYear=  await BookModel.find({year: pubYear})
    res.send({msg: selectedYear})
}


const getParticularBooks= async function (req, res) {
    let fetchData = req.body
    let partBooks= await BookModel.find(fetchData)
    res.send({msg: partBooks})
}


const getXINRBooks= async function (req, res) {
    let indPrice= await BookModel.find({$or: [{"prices.indianPrice":{$eq:"100INR"}},{"prices.indianPrice":{$eq:"200INR"}},{"prices.indianPrice":{$eq:"500INR"}}]})
    res.send({msg: indPrice})
}


const getRandomBooks= async function (req, res) {
    let randomSel= await BookModel.find({$or: [{totalPage:{$gt:500}}, {stockAvailable: true}]})
    res.send({msg: randomSel})
}
